...
export function demandeDepuisRequete(corps = {}) {
  ...
  return {
    ...
    terrain: {
      commune: { code: texte(commune.code), nom: texte(commune.nom) },
 ...
 * @param {Uint8Array} [plan] le plan de situation, quand il existera
 */
export async function preparerDossier(demande, phrase, plan = null) {
  let images = {};
  ...
  return {
    ...
    avertissements: avertir(demande, etatSignature, plan),
  };
}
 ...
 */
function avertir(demande, etatSignature, plan) {
  const liste = [];

  if (!plan) {
    liste.push("le plan de situation n'est pas joint : c'est la seule pièce exigée "
      + "à l'appui d'une demande de certificat d'urbanisme d'information (art. R*410-1)");
  }
