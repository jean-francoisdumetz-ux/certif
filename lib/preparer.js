...
export function demandeDepuisRequete(corps = {}) {
  ...
  const absents = [];
  // La reference est le numero de groupe de la Data Room, quatre chiffres —
  // decision du 19/08/2026. C'est elle qui fait le lien entre la demande qui
  // part et le certificat qui revient : 0042/1-2 se classe en 0042CU. On
  // complete les zeros de tete plutot que de refuser « 42 » : la regle du plan
  // de nommage repare, elle ne punit pas. Un identifiant non numerique, lui,
  // est refuse — c'est probablement l'ancienne habitude du numero d'etude.
  let reference = texte(corps.reference).replace(/\s+/g, '');
  if (!reference) absents.push('référence de dossier');
  else if (!/^\d{1,4}$/.test(reference)) {
    throw new Refus(400, 'la référence doit être le numéro de groupe de la Data Room',
      { recu: reference, attendu: 'un à quatre chiffres — exemple : 0042' });
  } else {
    reference = reference.padStart(4, '0');
  }
