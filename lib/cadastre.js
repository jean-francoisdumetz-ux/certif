...
/** Une parcelle, cherchée combinaison par combinaison. */
async function chercherUne(parcelle, commune) {
  ...
  for (const essai of tentatives(parcelle, commune)) {
    ...
    if (!r.ok) {
      // Une indisponibilité n'est pas une absence : on la retient, et on n'en
      // conclut pas que la parcelle n'existe pas. On cesse aussi d'essayer les
      // autres combinaisons — c'est le service qui répond mal, pas les
      // paramètres qui sont faux.
      journal.push({ parcelle: designation, essai, resultat: r.motif });
      if (r.panne) return { journal, incident: r.motif };
      continue;
    }
  ...
  journal.push({ parcelle: designation, resultat: 'introuvable' });
  return { journal };
}

/**
 * PLUSIEURS PARCELLES À LA FOIS, MAIS PAS TOUTES.
 *
 * Une parcelle coûte de une à quatre interrogations, et elles s'enchaînaient
 * parcelle après parcelle : quarante parcelles à un demi-tiers de seconde
 * l'appel, c'était vingt secondes sur les soixante dont dispose la fonction —
 * avant même que le plan ne soit demandé. Quatre parcelles de front divisent
 * l'attente par quatre.
 *
 * QUATRE, ET PAS QUARANTE. L'API Carto est un service public gratuit : lui
 * envoyer quarante requêtes simultanées pour gagner deux secondes serait se
 * servir en voisin peu délicat, et c'est aussi le meilleur moyen de récolter
 * les 5xx qu'on cherche justement à éviter.
 */
 ...
 *   route de diagnostic affiche quand une parcelle reste introuvable. Il reste
 *   dans l'ORDRE DE LA SAISIE quel que soit l'ordre des réponses : un journal
 *   dont les lignes s'entremêlent ne se lit plus.
 */
export async function geometriesParcelles(parcelles, commune) {
  const resultats = new Array(parcelles.length);
  let suivante = 0;
  ...
  const ouvrier = async () => {
    for (;;) {
      const i = suivante;
      suivante += 1;
      if (i >= parcelles.length) return;
      resultats[i] = await chercherUne(parcelles[i], commune);
    }
  };
  await Promise.all(Array.from({ length: Math.min(FRONT, parcelles.length) }, ouvrier));
  ...
  for (const r of resultats) {
    ...
    incident = incident || r.incident || null;
    if (r.trouvee) {
      trouvees.push(r.trouvee);
      anneaux.push(...r.trouvee.anneaux);
    }
  }
