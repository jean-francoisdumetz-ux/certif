 ...
 */
export async function construireDossier({ demande, cerfa, annexe, plan, images = {} }) {
  if (!cerfa) throw new Error('le Cerfa rempli est requis');

  const lettre = await construireLettre(demande, images);
  const parExemplaire = (await nombreDePages(cerfa))
  ...
  const debut = lettre.pages + 1;
  const pagination = {
    lettre: lettre.pages,
    parExemplaire,
    total: lettre.pages + 2 * parExemplaire,
    exemplaires: [
