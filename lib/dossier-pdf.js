...
export function nomFichier(demande, demandes = 1) {
  // Nommer la seule premiere commune d'un lot qui en couvre deux ferait croire
  // que tout part au meme endroit — et c'est le fichier que l'assistante voit
  // en premier. Plusieurs communes : on les compte au lieu d'en elire une.
  const noms = [...new Set((demande.terrains || [demande.terrain])
    .map((t) => t?.commune?.nom).filter(Boolean))];
  const commune = (noms.length > 1 ? `${noms.length}-COMMUNES` : (noms[0] || 'commune'))
