...
/**
 * Les fonds essayés, dans l'ordre.
 *
 * LE PREMIER EST LE PARCELLAIRE : c'est un extrait de plan cadastral que
 * l'étude veut, pas un plan de ville. Le PLAN IGN reste derrière, en cas de
 * couche indisponible — mieux vaut un plan de situation lisible qu'aucune
 * pièce.
 *
 * Les noms de couche et de style ne sont pas éprouvés d'ici : le bac à sable
 * n'atteint pas la Géoplateforme. D'où la liste plutôt qu'une valeur unique,
 * et le journal qui dit laquelle a répondu. Réglable par CERTIF_CARTE_COUCHES,
 * au format « COUCHE:style », séparés par des virgules.
 */
export const COUCHES = (process.env.CERTIF_CARTE_COUCHES
  || 'CADASTRALPARCELS.PARCELLAIRE_EXPRESS:PCI'
  + ',CADASTRALPARCELS.PARCELLAIRE_EXPRESS:normal'
  + ',GEOGRAPHICALGRIDSYSTEMS.PLANIGNV2:normal')
  .split(',').map((c) => {
    const [couche, style = 'normal'] = c.trim().split(':');
    return { couche, style };
  }).filter((c) => c.couche);
...
async function tuile(col, rang, zoom, fond = COUCHES[0]) {
...
export async function construirePlanSituation(
  ...
  // On essaie les fonds l'un après l'autre, sur la première tuile seulement :
  // inutile d'en charger trente pour découvrir que la couche n'existe pas.
  const suite = [...cadastre.journal];
  let fond = null;
  let images = null;
  let dernier = null;
  for (const candidat of COUCHES) {
    const essai = await chargerTuile(tuiles[0].col, tuiles[0].rang, zoom, candidat);
    dernier = essai;
    suite.push({
      fond: `${candidat.couche}:${candidat.style}`,
      resultat: essai.ok ? 'répond' : (essai.statut || essai.motif || 'aucune réponse'),
    });
    if (!essai.ok) continue;
    fond = candidat;
    images = [essai, ...await Promise.all(
      tuiles.slice(1).map((t) => chargerTuile(t.col, t.rang, zoom, candidat)))];
    break;
  }

  if (!fond) {
    return {
      erreur: `fond de plan indisponible (${dernier?.statut || dernier?.motif || 'aucune réponse'})`,
      journal: suite,
    };
  }
  ...
  dessinerTitre(page, f, demande, echelle, fond);
  dessinerLegende(page, f, echelle, cadastre.parcelles.length, fond);
  dessinerNord(page, f);
...
function dessinerTitre(page, f, d, echelle, fond) {
  const haut = A4.hauteur - MARGE;
  const cadastral = /CADASTRALPARCELS/i.test(fond?.couche || '');
  ecrire(page, cadastral ? 'Extrait de plan cadastral' : 'Plan de situation', {
    x: MARGE, y: haut - 14, taille: 16, fonte: f.romainGras,
  });
  ecrire(page, 'Plan de situation — annexe à la demande de certificat d’urbanisme, '
    + 'Cerfa n° 13410*13', {
...
function dessinerLegende(page, f, echelle, nombre, fond) {
  const y = CADRE.y - 12;
  ...
  const cadastral = /CADASTRALPARCELS/i.test(fond?.couche || '');
  const source = cadastral
    ? 'Parcellaire Express © IGN — DGFiP · Géoplateforme · Contour : API Carto IGN'
    : 'Fond de plan © IGN — Géoplateforme · Parcellaire : PCI Express, API Carto IGN';
